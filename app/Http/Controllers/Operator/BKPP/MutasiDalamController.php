...
class MutasiDalamController extends Controller
{
    public $pengajuan;
    public $dokumen;
    public $komentar;
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()) {
           ...
           if($status) {
               $data['table'] = $pengajuan->where('id_katpengajuan', '8')->where('status', $status)->with('user')->paginate();
            }else {
               $data['table'] = $pengajuan->where('id_katpengajuan', '8')->with('user')->paginate();
           }
           return \view('bkpp.mutasi._data_mutasi_dalam', $data);
        }
        $data['title'] = "Mutasi dalam";
        $data['pengajuan_baru'] = $this->pengajuan->Query()->where('id_katpengajuan', '8')->where('status', 'pending')->count();
        $data['pengajuan_diproses'] = $this->pengajuan->Query()->where('id_katpengajuan', '8')->where('status', 'diproses')->count();
        $data['pengajuan_diterima'] = $this->pengajuan->Query()->where('id_katpengajuan', '8')->where('status', 'selesai')->count();
        $data['pengajuan_ditloak'] = $this->pengajuan->Query()->where('id_katpengajuan', '8')->where('status', 'ditolak')->count();
        return view('bkpp.mutasi.index', $data);
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if($request->ajax()) {
            ...
            return view('bkpp.mutasi._komentar', $data);
        }
        ...
        return view('bkpp.mutasi.show_mutasi_dalam', $data);
    }
