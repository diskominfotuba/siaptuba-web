...
class PangkatregulerController extends Controller
{
    public $pengajuan;
    public $dokumen;
    public $komentar;
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()) {
           ...
           if($status) {
               $data['table'] = $pengajuan->where('id_katpengajuan', '1')->where('status', $status)->with('user')->paginate();
            }else {
               $data['table'] = $pengajuan->where('id_katpengajuan', '1')->with('user')->paginate();
           }
           return \view('bkpp.kenaikanpangkat._data_reguler', $data);
        }
        $data['title'] = "Kenaikan Pangkat Reguler/Staf/Pelaksana";
        $data['pengajuan_baru'] = $this->pengajuan->Query()->where('id_katpengajuan', '1')->where('status', 'pending')->count();
        $data['pengajuan_diproses'] = $this->pengajuan->Query()->where('id_katpengajuan', '1')->where('status', 'diproses')->count();
        $data['pengajuan_diterima'] = $this->pengajuan->Query()->where('id_katpengajuan', '1')->where('status', 'selesai')->count();
        $data['pengajuan_ditloak'] = $this->pengajuan->Query()->where('id_katpengajuan', '1')->where('status', 'ditolak')->count();
    ...
    public function show(Request $request, $id)
    {
        ...
        return view('bkpp.kenaikanpangkat.show_reguler', $data);
    }
