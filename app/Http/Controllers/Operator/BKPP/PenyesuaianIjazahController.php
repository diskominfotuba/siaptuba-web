...
class PenyesuaianIjazahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()) {
           ...
           if($status) {
               $data['table'] = $pengajuan->where('id_katpengajuan', '5')->where('status', $status)->with('user')->paginate();
            }else {
               $data['table'] = $pengajuan->where('id_katpengajuan', '5')->with('user')->paginate();
           }
           return \view('bkpp.penyesuaianijazah._data_table', $data);
        }
        $data['title'] = "Penyesuaian Ijasah";
        $data['pengajuan_baru'] = $this->pengajuan->Query()->where('id_katpengajuan', '5')->where('status', 'pending')->count();
        $data['pengajuan_diproses'] = $this->pengajuan->Query()->where('id_katpengajuan', '5')->where('status', 'diproses')->count();
        $data['pengajuan_diterima'] = $this->pengajuan->Query()->where('id_katpengajuan', '5')->where('status', 'selesai')->count();
        $data['pengajuan_ditloak'] = $this->pengajuan->Query()->where('id_katpengajuan', '5')->where('status', 'ditolak')->count();
        return view('bkpp.penyesuaianijazah.index', $data);
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if($request->ajax()) {
            ...
            return view('bkpp.penyesuaianijazah._komentar', $data);
        }
        ...
        return view('bkpp.penyesuaianijazah.show', $data);
    }
