...
class MutasiMasukController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()) {
           ...
           if($status) {
               $data['table'] = $pengajuan->where('id_katpengajuan', '6')->where('status', $status)->with('user')->paginate();
            }else {
               $data['table'] = $pengajuan->where('id_katpengajuan', '6')->with('user')->paginate();
           }
           return \view('bkpp.mutasi._data_mutasi_masuk', $data);
        }
        $data['title'] = "Mutasi masuk";
        $data['pengajuan_baru'] = $this->pengajuan->Query()->where('id_katpengajuan', '6')->where('status', 'pending')->count();
        $data['pengajuan_diproses'] = $this->pengajuan->Query()->where('id_katpengajuan', '6')->where('status', 'diproses')->count();
        $data['pengajuan_diterima'] = $this->pengajuan->Query()->where('id_katpengajuan', '6')->where('status', 'selesai')->count();
        $data['pengajuan_ditloak'] = $this->pengajuan->Query()->where('id_katpengajuan', '6')->where('status', 'ditolak')->count();
    ...
    public function show(Request $request, $id)
    {
        ...
        return view('bkpp.mutasi.show_mutasi_masuk', $data);
    }
