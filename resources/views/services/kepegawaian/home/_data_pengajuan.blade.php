@forelse ($table as $key => $tb)
    <a href="{{ route('services.kepegawaian.pengajuan', $tb->id) }}" class="item">
        <div class="detail">
            {{ $tb->layanan->nama_layanan }}
        </div>
        <div class="right">
            <div>
                @if ($tb->status == 'pending' || $tb->status == 'terverifikasi operator' || $tb->status == 'diajukan')
                    <span class="badge bg-warning">{{ $tb->status }}</span>
                @elseif($tb->status == 'diproses')
                    <span class="badge bg-info">{{ $tb->status }}</span>
                @elseif($tb->status == 'diterima')
                    <span class="badge bg-primary">{{ $tb->status }}</span>
                @elseif($tb->status == 'ditolak bkpp' || $tb->status == 'ditolak operator')
                    <span class="badge bg-danger">{{ $tb->status }}</span>
                @else
                    <span class="badge bg-success">{{ $tb->status }}</span>
                @endif
            </div>
        </div>
    </a>
