...
<div class="mt-1">
    <table class="table table-responsive">
        <tbody>
            <tr>
            <td>
                <strong class="text-primary">Nama file: {{ $item->nama_file }}</strong>
            </td>
        </tr>
        <tr>
            <td>
                @if ($item->kategori_dokumen == 'berkala')
                    <a class="btn btn-primary btn-sm" href="{{ route('pdfviewer', ['dir' => 'berkala','filename' => $item->file]) }}">Lihat file</a>
                    @else
                    <a class="btn btn-primary btn-sm" href="{{ route('pdfviewer', ['dir' => 'dokumen_kepegawaian','filename' => $item->file]) }}">Lihat file</a>
                @endif
                <span class="btn btn-danger btn-sm" onclick="hapusFile('{{ $item->id }}')">Hapus file</span>
            </td>
        </tr>
        </tbody>
    </table>
