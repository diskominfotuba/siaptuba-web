<table class="table w-100">
    ...
    <tbody>
        @forelse ($table as $item)
            <tr data-id="{{ $item->id }}">
                <td class="namaFile w-100"><a
                        href="{{ route('pdfviewer', ['dir' => 'bkpp', 'filename' => $item->file]) }}">{{ $item->nama_file }}</a>
                </td>
                <td class="text-nowrap">
                    <span class="btn btn-primary btn-sm" data-dismiss="modal" aria-label="Close"
                        onclick="pilihFile('{{ $item->id }}')">Pilih file</span>
                    <a class="btn btn-secondary btn-sm"
                        href="{{ route('pdfviewer', ['dir' => 'bkpp', 'filename' => $item->file]) }}">Lihat file</a>
                </td>
            </tr>
        @empty
            <tr>
