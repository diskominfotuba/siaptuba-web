@forelse ($table as $tb)
    <div class="goals mb-1" onclick="getCuty({{ $tb }})" data-toggle="modal" data-target="#modal-show">
        <div class="item">
            <div class="in">
                <div>
                    <h4>Tgl masuk: {{ \Carbon\Carbon::parse($tb->tanggal_masuk)->isoFormat('dddd, D MMMM YYYY') }}</h4>
                    <p>Cuti dimulai dari {{ $tb->tanggal_awal }} sampai {{ $tb->tanggal_akhir }}</p>
                    <p>Jumlah cuti {{ $tb->jumlah_izin }} hari</p>
                    @if ($tb->approval_status == 'pending')
                        <span class="badge bg-warning">{{ $tb->approval_status }}</span>
                    @elseif($tb->approval_status == 'disetujui')
                        <span class="badge bg-success">{{ $tb->approval_status }}</span>
                    @else
                        <span class="badge bg-danger">{{ $tb->approval_status }}</span>
                    @endif
                </div>
            </div>
        </div>
