@forelse ($table as $tb)
<a href="javascript:void()" onclick="showTask('{{ $tb->id }}')" data-toggle="modal" data-target="#showTask" class="item">
    <div class="detail">
        @if ($tb->bukti_dukung)
        <img src="{{ route('lampiran-ekin', ['opd' => $tb->opd_id, 'year' => $tb->created_at->format('Y'), 'filename' => $tb->bukti_dukung]) }}" alt="img" class="image-block imaged w48">
        @else
        <img src="" alt="gambar" class="image-block imaged w48">
        @endif
        <div>
            @if (auth()->user()->role == 'oprator')
            <strong>{{ $tb->user->nama }}</strong>
            <div class="title mb-1">{{ $tb->nama_tugas }}</div>
            <p>{{ $tb->created_at->format('d-m-Y H:i:s') }}</p>
            @else
            <strong>{{ $tb->nama_tugas }}</strong>
            <p>{{ $tb->created_at->format('d-m-Y H:i:s') }}</p>
            @endif
        </div>
    </div>
...
<div class="container mt-2">
    <div class="row justify-content-center">
        <div>Showing {{ $table->firstItem() }} to {{ $table->lastItem() }} of {{ $table->total() }}entries</div>
