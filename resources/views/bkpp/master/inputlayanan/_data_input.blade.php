    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td class="nama">{{ $tb->nama_input }}</td>
                <td>
                    <input class="form-check-input" type="checkbox" onclick="updateStatus({{ $tb->id }}, event);"
                        {{ $tb->required == 'y' ? 'checked' : '' }}>
                    <label class="form-check-label" for="defaultCheck3"> Wajib </label>
                </td>
                <td>
                    <button class="btn btn-sm btn-warning"
                        onclick="updateKategori('{{ $tb->id }}', '{{ $tb->nama_input }}')">edit</button>
