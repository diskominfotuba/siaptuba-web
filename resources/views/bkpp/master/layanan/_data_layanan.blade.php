    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td class="nama">{{ $tb->nama_layanan }}</td>
                <td>
                    <a href="{{ route('bkpp.master.inputlayanan', ['kategori_id' => request()->kategori_id, 'layanan_id' => $tb->id]) }}"
                        class="btn btn-primary btn-sm">Lihat Berkas</a>
                    <a href="{{ route('bkpp.master.periode', ['kategori_id' => request()->kategori_id, 'layanan_id' => $tb->id]) }}"
                        class="btn btn-info btn-sm">Periode</a>
                    <button class="btn btn-sm btn-warning"
                        onclick="updateKategori('{{ $tb->id }}', '{{ $tb->nama_layanan }}')">edit</button>
