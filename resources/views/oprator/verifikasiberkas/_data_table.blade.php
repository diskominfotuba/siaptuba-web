    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td>{{ $tb->layanan->nama_layanan }}</td>

                @if ($tb->status == 'diajukan')
                    <td><span class="badge bg-warning">{{ $tb->status }}</span></td>
                @elseif($tb->status == 'terverifikasi operator')
                    <td><span class="badge bg-success">{{ $tb->status }}</span></td>
                @elseif($tb->status == 'ditolak operator')
                    <td><span class="badge bg-danger">{{ $tb->status }}</span></td>
                @else
                    <td><span class="badge bg-secondary">{{ $tb->status }}</span></td>
                @endif
                <td>
                    <a href="{{ route('operator.verifikasi_berkas.show', $tb->id) }}"
