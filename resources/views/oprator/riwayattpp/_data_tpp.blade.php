    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td>{{ $tb->nama ?? '-' }}</td>
                <td>{{ formatRp($tb->tpp ?? '0') }}</td>
                <td>{{ formatRp($tb->lastTpp->tpp_berjalan ?? '0') }}</td>
                <td>{{ !empty($tb->lastTpp->created_at) ? \Carbon\Carbon::parse($tb->lastTpp->created_at)->format('d-m-Y') : '-' }}
                </td>
                <td><a href="{{ route('operator.riwayattpp.show', ['user_id' => $tb->id]) }}"
                        class="btn btn-primary btn-sm">Riwayat</a>
                </td>
            </tr>
        @empty
            <tr>
