<table class="table table-bordered">
    <thead>
        <tr>
            <th>#</th>
            <th>Nama</th>
            <th>Keterangan (tdk hadir/telat)</th>
            <th>Pengurangan</th>
            <th>Tanggal</th>
        </tr>
    </thead>
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                <td>{{ $key + 1}}</td>
                <td>{{ $tb->user->nama ?? '-' }}</td>
                <td>{{ $tb->jumlah_menit }}</td>
                <td>- {{ formatRp($tb->pengurangan ?? '-') }}</td>
        ...
        <tr class="table-primary">
            ...
            <td colspan="2">{{ formatRp($total_pengurangan) }}</td>
        </tr>
        <tr class="table-primary">
            <td colspan="3">Estimasi TPP Diterima:</td>
            <td colspan="2">{{ formatRp($estimasi_tpp_diterima) }}</td>
        </tr>
    </tbody>
</table>
