<table class="table table-bordered">
    <thead>
        <tr>
            <th>#</th>
            <th>Nama Kegiatan</th>
            <th style="width: 190px">Tgl Mulai Kegiatan</th>
            <th style="width: 190px">Tgl Akhir Kegiatan</th>
            <th style="width: 224px">Aksi</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr>
            ...
            <td>{{ \Illuminate\Support\Carbon::parse($tb->tanggal_mulai)->isoFormat('DD-MM-YYYY HH:mm') }}</td>
            <td>{{ \Illuminate\Support\Carbon::parse($tb->tanggal_akhir)->isoFormat('DD-MM-YYYY HH:mm') }}</td>
            <td>
                <a href="{{ route('oprator.kegiatan.show', ['id' => $tb->id]) }}" class="btn btn-primary btn-sm">Generate QR Code</a>
                <a href="{{ route('oprator.kegiatan.tamu-undangan', ['id' => $tb->id]) }}" class="btn btn-warning btn-sm">Lihat</a>
            </td>
        </tr>
        @empty
