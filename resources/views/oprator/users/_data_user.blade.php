<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <select class="form-select" style="width: 120px; border-color: #696cff" onchange="updateStatusPegawai(this.value, '{{ $tb->id }}')" name="status_pegawai">
                    <option value="asn" {{ $tb->status_pegawai == 'asn' ? 'selected' : '' }}>ASN</option>
                    <option value="non_asn" {{ $tb->status_pegawai == 'non_asn' ? 'selected' : '' }}>NON ASN</option>
                    @if (auth()->user()->opd_id == '20')
                    <option value="to" {{ $tb->status_pegawai == 'to' ? 'selected' : '' }}>Tenaga Outsorcing</option>
                    @endif
                </select>
            </td>
