...
@push('js')
    ...
    <script>
        ...
        function showPresensi(url, waktu, nama_pegawai, tanggal, jam_presensi, latlong) {
            if (waktu === 1) {
                $('.modal-title').html('Detail Presensi Pagi');
                $('#photoAbsen').attr('src', url)
                $('input[name=nama]').val(nama_pegawai);
                $('input[name=tanggal]').val(tanggal);
                $('input[name=jam_masuk]').val(jam_presensi);
                $('input[name=latlong]').val(latlong);
            } else {
            ...
            const lat = latlong.substring(10, '');
            const long = latlong.substring(11);
